...
interface TikTokInvoiceDialogProps {
  payments: TikTokPayment[];
  filterLabel?: string;
}

export function TikTokInvoiceDialog({ payments, filterLabel }: TikTokInvoiceDialogProps) {
  ...
  const handleGenerate = async (data: InvoiceFormData, action: 'download' | 'print' | 'share') => {
    ...
    if (payments.length === 0) {
      toast.error('No payments to include in invoice');
      return;
    }
    ...
    try {
      ...
      const title = filterLabel || 'TikTok Payments';
      ...
      await supabase.from('invoices').insert({
        user_id: user.id,
        project_id: '00000000-0000-0000-0000-000000000000', // placeholder for TikTok invoices
      ...
      if (action === 'download') {
        pdf.save(`${invoiceNumber}_TikTok_Payments.pdf`);
        ...
        if (navigator.share && navigator.canShare({ files: [file] })) {
          await navigator.share({ files: [file], title: `Invoice ${invoiceNumber}`, text: `TikTok Payment Invoice` });
          ...
          <form className="space-y-4">
            <div className="rounded-lg border p-4 bg-muted/50">
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <span className="font-medium">{payments.length} records</span>
              </div>
            </div>
            ...
            <div className="space-y-2 rounded-lg border p-4">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Payments ({payments.length} items)</span>
